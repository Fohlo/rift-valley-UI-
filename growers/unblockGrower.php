...
$tm = $grower_i2['Message'];
	$ti = $grower_i2['ExceptionMessage'];
		$grower_i2 = json_encode($booking_id, true);
		$error = "";//"Error: call to URL $url failed with status $status, response $grower_i2";
		//print($error);		//die("Error: call to URL $url failed with status $status, response $json_response, curl_error " . curl_error($curl) . ", curl_errno " . curl_errno($curl));
...
$tm = $grower_i2['Message'];
	$ti = $grower_i2['ExceptionMessage'];
		$grower_i2 = json_encode($grower_i2, true);
		$error = "Error: call to URL $url failed with status $status, response $grower_i2";
		//print($error);		//die("Error: call to URL $url failed with status $status, response $json_response, curl_error " . curl_error($curl) . ", curl_errno " . curl_errno($curl));
		$_SESSION['capnum'] = "Enter the correct Grower Number!";
		$_SESSION['capnums'] = "";
...
if(isset($_POST['addbooking'])){
	...
	$url = "Growers/Unblock/$user";    
	$content = json_encode($data);
list($response,$status) = ApiConnect::Post($url,$content);

	if ( $status == 200 ) {
	
$msg = "Unblocking of Grower Successful";
	}else{
$tm = $response['Message'];
 ...
 <div class="main-page">
                        <div class="container-fluid">
                            <div class="row page-title-div">
                                <div class="col-md-6">
                                    <h2 class="title">Unblock Grower</h2>
                                </div>
                            ...
                            <div class="row breadcrumb-div">
                                <div class="col-md-6">
                                    <ul class="breadcrumb">
            							...
            							<li class="active">Unblock Grower</li>
            						</ul>
                                </div>
        ...
        <form method="post" name = "Form" action="">
          <div class="card">
            <div class="card-header">
              <strong>Unblock Grower</strong>
              <a href="/rvc_dev/growers/index.php" class="btn btn-primary btn-sm pull-right"><i style="color:white;" class="fa fa-align-justify"></i>Growers</a>
	...
	<div class="col-md-6 col-sm-6 col-xs-12">
		<textarea class="form-control" name="comments" id="comments"  placeholder="Add Summary Comments" ></textarea>
              ...
              <div class="card-footer"  align="left">
                ...
                <button type="submit"  name="addbooking" class="btn btn-sm btn-primary"><i class="fa fa-check"></i>Unblock Grower</button>
...
<script>
  $(document).ready(function(){
	  ...
	  const Url: "Growers/GetGrower/$grower_n1";
	   $.ajax({
    type: "GET",
    url: Url,
    dataType: 'json',
...
<script type = "text/javascript">
  ...
  $.ajax({
    url: "Growers/GetGrower/$grower_n1",
    dataType: 'json',
